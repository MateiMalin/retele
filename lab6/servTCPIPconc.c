...
int main()
{
    ...
    while (1)
    {
        ...
        if ((pid = fork()) == -1)
        {
            ...
            close(client); // Inchidem clientul si continuam
            continue;
        }
        else if (pid == 0)
        {
            ...
            bzero(msg, 100);
            printf("[server-fiu %d] Asteptam mesajul...\n", getpid());
            fflush(stdout);
            ...
            close(client);
            exit(0); // Foarte important: terminam procesul fiu
        }
        else
        {
            close(client);
